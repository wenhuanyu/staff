<template>
	<view>
		<uni-popup ref="popup" type="bottom" background-color="#fff" @touchmove.stop.prevent>
			<view class="popup_title">选择部门</view>
			<view class="searbox">
				...
				<input @input="searchName" type="text" placeholder-style="color:#999;" 
					:class="keyword ? 'fontactive' : ''" v-model="keyword" placeholder="部门名称" class="li_box" />
			</view>
			<scroll-view scroll-y class="scrollbox" @scrolltolower="loadMoredata" lower-threshold="30">
				<view class="liwarp" v-for="(item,index) in departmentlist" :key="index" @click="selectItem(item,index)">
					<view class="warpleft">
						<view class="name">名称:{{item.name}}</view>
					</view>
					<label class="radio">
						<radio value="r1" color="#008EFF" :checked="departmentObj.id == item.id" />
					</label>
				</view>
				<view class="noData" v-if="departmentlist.length == 0">暂无更多</view>
			</scroll-view>
	...
	export default{
		...
		methods:{
			...
			sureSelect() {
				...
				this.$refs.popup.close()
				this.$emit('getCustomerinfo',this.departmentObj)
			},
			hideSelect() {
				this.$refs.popup.close()
			}
