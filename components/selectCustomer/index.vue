<template>
	<view>
		<uni-popup ref="popup" type="bottom" background-color="#fff" @touchmove.stop.prevent>
			...
			<view class="searbox">
				<uni-search-bar v-model="keyword" @input="searchName"  placeholder="请输入搜索关键词"  @cancel="searchName" @clear="searchName"></uni-search-bar>
			</view>
			<scroll-view scroll-y class="scrollbox" @scrolltolower="loadMoredata" lower-threshold="30">
				<view class="liwarp" v-for="(item,index) in customerlist" :key="index" @click="selectItem(item,index)">
					...
					<label class="radio">
						<radio value="r1" :checked="preseCustomerObj.id == item.id" />
					</label>
				</view>
...
<script>
	import { netPrevCustomer } from '@/api/api.js'
	...
	export default{
		...
		methods:{
			...
			getList() {
				...
				netPrevCustomer(params).then(res=>{
