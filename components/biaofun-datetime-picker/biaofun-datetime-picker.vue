<template>
	<view class="datatime">
		<picker mode="multiSelector" :range="range" range-key="text" @change="change" @columnchange="columnchange" :value="value" :disabled="disabled">
			<view class="content" :class="{ placeholder: !dateStr,'stylecenter':styleCenter }">
				<text :class="{'valueActive':dateStr}">{{ dateStr ? dateStr : placeholder }}</text>
			</view>
		</picker>
	</view>
</template>
...
export default {
	...
	watch: {
		...
		defaultValue(newValue,oldValue) {
			console.log(newValue,oldValue,'监听值变化')
			// 设置默认值
	...
	methods: {
		 ...
		 */
		setDefaultValue() {
			console.log(this.defaultValue,'默认值')
			// 默认日期
			let dtDefault;
