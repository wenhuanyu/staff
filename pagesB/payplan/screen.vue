	...
	<view class="">
		<uni-drawer ref="showRight" mode="right" :width="drWidth">
			...
			<scroll-view scroll-y class="scrollbox">
				<view class="scr_warp">
					<uni-collapse ref="collapse" v-model="activeNames" @change="onChange">
						<uni-collapse-item title="关联合同" name="b">
							<view class="listwarp">
								<view class="li" :class="contractObj ?'liactive':''" @click.stop="toSelectContract">
									{{contractObj?contractObj.name:'关联合同'}}
									<view class="clearStaff" v-if="contractObj" @click.stop="clearContract">×</view>
								</view>
							</view>
						</uni-collapse-item>
						<uni-collapse-item title="回款计划状态" name="a">
							<view class="listwarp">
								<view 
									class="li" 
									:class="index == status ?'liactive':''" 
			...
			<view class="screenbot">
				...
				<view class="screenbtn surebtn" @click="sureQuery">确认</view>
			</view>
		</uni-drawer>
