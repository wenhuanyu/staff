<template>
	<view class="">
		<view class="top">
			...
			<view class="text">{{info.create_staff.name}}的请假</view>
		...
		<view class="section">
			...
			<view class="sec_li"> <text>请假类型</text> {{info.type}}</view>
			<view class="sec_li"> <text>开始时间</text> {{info.start_time}}</view>
			<view class="sec_li"> <text>结束时间</text> {{info.end_time}}</view>
			<view class="sec_li"> <text>请假时长</text> {{info.hour}}小时</view>
			<view class="sec_li"> <text>请假事由</text> {{info.remarks}}</view>
...
<script>
	import { netLeaveDetail, netApprovalDetail } from '@/api/api.js'
	...
	export default{
		...
		methods:{
			getDetail() {
				netLeaveDetail({id:this.id}).then(res=>{
					this.info = res.data
				})
			},
			getApprovalList() {
			...
			toPass() {
				uni.navigateTo({ //status   1成功   2失败
					url: '/pages/examine/examine?id=' + this.id + '&type=leave&status=1'
				})
			},
			//拒绝
			toRefuse() {
				uni.navigateTo({ //status   1成功   2失败
					url: '/pages/examine/examine?id=' + this.id + '&type=leave&status=2'
				})
			}
		}
	}
