	...
	<view>
		...
		<view class="inp_warp">
			...
			<view class="form_right">
				<input type="text" v-model="name" placeholder="姓名" disabled class="rightinput" :class="name ? 'valueActive' : '' ">
			</view>
		</view>
		<view class="inp_warp">
			<view class="inp_title key">部门<span>*</span></view>
			<view class="form_right">
				<input type="text" v-model="department_name" disabled placeholder="部门" class="rightinput" :class="department_name ? 'valueActive' : '' ">
			</view>
		</view>
		<view class="inp_warp">
			<view class="inp_title key">职位<span>*</span></view>
			<view class="form_right">
				<input type="text" v-model="post" disabled placeholder="职位" class="rightinput" :class="post ? 'valueActive' : '' ">
			</view>
		</view>
		<view class="inp_warp" v-if="formid == 113 || formid == 120">
...
<script>
	...
	export default{
		...
		onLoad(options) {
			...
			this.department_name = uni.getStorageSync('userinfo').department_name
			this.post = uni.getStorageSync('userinfo').post
		...
		methods:{
			...
			toSubmit() {
				...
				if(this.formid == 113 || this.formid == 120) {
					data.num = this.num
				}
				data.name = this.name
