			...
			<view class="coll_header" v-for="(item,index) in collecList" :key="index">
				...
				<view class="coll_li">
					<picker @change="(e)=>{changeColltime(e,index)}" mode="date">
						<view class="uni-input" :class="{'uni-input-a':item.time}">{{item.time ? item.time : '回款时间'}}</view>
					</picker>
				</view>
			</view>
		</view>
...
<script>
	...
	import { netSetForm, netPrevCustomer, netLianxiren, 
	netAutoOrderNumber, netCreateContract,  netQuotationDetail, netTeamList, netDistributionType, netApprovalProcess, netBusinessDetail } from '@/api/api.js'
