	...
	<view>
		...
		<view class="searbox">
			<view class="censear">
				...
				<input type="text" placeholder="请输入关键词搜索" @input="queryName" class="lself" placeholder-style="color:#999">
			</view>
		</view>
		...
		<view class="botwarp">
			...
			<scroll-view scroll-x class="bot_scrollbox">
				<view class="botlist">
					<view class="botli" >
						<view class="botname">{{checkobj.name}}</view>
					</view>
				</view>
			</scroll-view>
...
<script>
	import { netStaffList } from '@/api/api.js'
	...
	export default{
		...
		methods:{
			...
			sureCustomer() {
				this.$store.commit('STAFFTWO',this.checkobj)
				uni.navigateBack({
					delta:1
				})
			}
