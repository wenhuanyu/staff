<template>
	<view>
		...
		<view style="background:#fff;">
			...
			<view v-if="event_process == 2">
				<view class="inp_warp" v-if="car_type == '公司车' || car_type == '私家车'">
					<view class="inp_title key">开始公里数<span>*</span></view>
					<view class="form_right">
						<input type="number" @blur="checkMileage" v-model="start_mileage" placeholder-style="font-size:24rpx;color:#999;" placeholder="请输入结束公里数" class="rightinput" :class="start_mileage ? 'valueActive' : '' ">
					</view>
				</view>
				<view class="inp_warp" v-if="car_type == '公司车' || car_type == '私家车'">
					<view class="inp_title key">结束公里数<span>*</span></view>
					<view class="form_right">
						<input type="number" @blur="checkendMileage" v-model="end_mileage" placeholder-style="font-size:24rpx;color:#999;" placeholder="请输入结束公里数" class="rightinput" :class="end_mileage ? 'valueActive' : '' ">
					</view>
				</view>
			</view>
			<view v-if="event_process == 4">
				<view class="inp_warp" v-if="car_type == '公司车' || car_type == '私家车'">
					<view class="inp_title key">开始公里数<span>*</span></view>
					<view class="form_right">
						<input type="digit" v-model="end_mileage" placeholder-style="font-size:24rpx;color:#999;" disabled placeholder="请输入结束公里数" class="rightinput" :class="end_mileage ? 'valueActive' : '' ">
					</view>
				</view>
				<view class="inp_warp" v-if="car_type == '公司车' || car_type == '私家车'">
					<view class="inp_title key">结束公里数<span>*</span></view>
					<view class="form_right">
						<input type="digit" v-model="back_mileage" placeholder-style="font-size:24rpx;color:#999;" placeholder="请输入结束公里数" class="rightinput" :class="back_mileage ? 'valueActive' : '' ">
					</view>
				</view>
			</view>
			<!-- 上传图片 -->
			<upload-img :flag="car_type == '公司车' || car_type == '私家车'" :imgList="imgList" istype="camera" @uploadImg="uploadImg" @delImg="delImg"></upload-img>
...
<script>
	...
	import { netAddSignin, netLngTransAddress } from '@/api/api.js'
	...
	export default {
		...
		onLoad(options) {
			...
			if(options.type == 2) {
				...
				this.end_mileage = options.endmileage
			}
		...
		methods: {
			...
			toAdd() {
				...
				if((this.car_type == '公司车' || this.car_type == '私家车') && imgList.length == 0) {
					uni.showToast({
						title:'请上传图片',
						icon:'none'
					})
				...
				if(event_process == 2) {
					...
					params = {
						...
						event_id,
						event_process,
						back_mileage:this.back_mileage
					}
				}
