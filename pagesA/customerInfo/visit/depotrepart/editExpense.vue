	...
	<view>
		...
		<view v-if="type != '电话售后'">
			<view class="fllow_form">
				<view class="key">服务费用<span>*</span></view>
				<view class="form_right">
		...
		<view class="fllow_form" style="margin-top:24rpx;">
			<view class="key">配件费用<span>*</span></view>
			<view class="form_right">
...
<script>
	...
	export default{
		...
		methods:{
			...
			getServe() {
				netTemplateList({type:'服务费用'}).then(res=>{
			...
			getDetail() {
				netWorkOrderDetail({id:this.id}).then(res=>{
					...
					if(res.data.tax == '不含税'){
						...
						this.isHan = 1
						let str = res.data.tax
						let index = str.indexOf('%')
						console.log(str.slice(1,index),'截取含税')
					}
					//计算 配件费用
					this.handlePart()
					//处理收款 帐号
					if(res.data.collection_account) {
						this.handleAccount(res.data.collection_account)
					}else{
						this.accountIndex = null
					}
					this.imgList = res.data.collection_img
				})
			},
			handleAccount(data) {
